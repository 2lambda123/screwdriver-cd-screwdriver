...
describe('Register Unit Test Case', () => {
  const expectedPlugins = [
    '@hapi/inert', '@hapi/vision', '../../plugins/status',
    '../../plugins/versions', '../../plugins/logging', '../../plugins/swagger',
    '../../plugins/template-validator', '../../plugins/command-validator',
    '../../plugins/promster', '../../plugins/metrics', '../../plugins/ratelimit'
  ];
  const resourcePlugins = [
    '../../plugins/auth',        '../../plugins/banners',
    '../../plugins/builds',      '../../plugins/buildClusters',
    '../../plugins/collections', '../../plugins/commands',
    '../../plugins/events',      '../../plugins/jobs',
    '../../plugins/pipelines',   '../../plugins/secrets',
    '../../plugins/stages',      '../../plugins/templates',
    '../../plugins/tokens',      '../../plugins/users',
    '../../plugins/webhooks',    '../../plugins/stats',
    '../../plugins/isAdmin',     '../../plugins/shutdown',
    '../../plugins/release',     '../../plugins/validator',
    '../../plugins/processHooks'
  ];
  const authPlugins = [
    '@hapi/bell', '@hapi/cookie', '@hapi/crumb', 'hapi-auth-bearer-token',
    'hapi-auth-jwt2'
  ];
  const pluginLength =
      expectedPlugins.length + resourcePlugins.length +
      authPlugins.length; // for server.register of auth Plugins;
  const mocks = {};
  const config = {
  ...
  it('registered all the default plugins', async () => {
    ...
    expectedPlugins.forEach(plugin => {
      serverMock.register.calledWith(
          {plugin : mocks[plugin], routes : {prefix : '/v4'}});
    });
  });
  ...
  it('registered notifications plugins', async () => {
    ...
    const notificationPlugins = [
      'screwdriver-notifications-email', 'screwdriver-notifications-slack'
    ];
  ...
  it('registered scoped notifications plugins', async () => {
    ...
    main.__set__('requireNotificationPlugin',
                 configMock => mocks[configMock.scopedPackage]);
  ...
  it('registered coverage as resource plugin if configured', async () => {
    ...
    resourcePlugins.forEach(plugin => {
      Assert.calledWith(
          serverMock.register,
          {plugin : mocks[plugin], options : {}, routes : {prefix : '/v4'}});
    });
  });
  ...
  it('bubbles failures up', () => {
    ...
    return main(serverMock, config)
        ...
        .catch(err => { Assert.equal(err.message, 'failure loading'); });
  });
