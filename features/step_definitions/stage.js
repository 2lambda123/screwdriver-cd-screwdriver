...
Given(
    ...
    {timeout : TIMEOUT}, async function step(branchName, stageName, table) {
...
When(
    ...
    {timeout : TIMEOUT}, function step(jobName, branchName) {
      const jobId =
          jobName
              ? Object.values(this.jobs).find(val => val.name === jobName).id
              : this.jobId;
...
Then(
    ...
    {timeout : TIMEOUT}, async function step(stage, stageBuildStatus) {
      const config = {
...
Then(/^the "([^"]*)" job is started$/, {timeout : TIMEOUT},
...
Then(/^the "([^"]*)" job on branch "([^"]*)" is not started/,
     {timeout : TIMEOUT}, async function step(jobName, branchName) {
