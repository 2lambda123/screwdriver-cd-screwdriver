...
async function registerDefaultPlugins(server) {
  const plugins = [
    '@hapi/inert', '@hapi/vision', '../plugins/status', '../plugins/versions',
    '../plugins/logging', '../plugins/swagger', '../plugins/template-validator',
    '../plugins/command-validator', '../plugins/promster', '../plugins/metrics',
    '../plugins/ratelimit'
  ].map(plugin => require(plugin));

  return plugins.map(async pluginObj => server.register(
                         {plugin : pluginObj, routes : {prefix : '/v4'}}));
}
 ...
 * @param  {Object}    server            Hapi server object
 * @param  {Object}    config            Configuration object for resource
 *     plugins
 */
async function registerResourcePlugins(server, config) {
  const plugins = [
    'auth',        'banners', 'builds',   'buildClusters', 'collections',
    'commands',    'events',  'jobs',     'pipelines',     'templates',
    'tokens',      'secrets', 'stages',   'users',         'webhooks',
    'stats',       'isAdmin', 'shutdown', 'release',       'validator',
    'processHooks'
  ];
...
async function registerAuthPlugins(server, config) {
  const plugins = [
    '@hapi/bell', '@hapi/cookie', 'hapi-auth-bearer-token', 'hapi-auth-jwt2',
    '@hapi/crumb'
  ];
  const crumbOptions = {
    '@hapi/crumb' : {
      cookieOptions : {isSecure : config.auth && config.auth.https},
      restful : true,
      skip : request =>
          // Skip crumb validation when the request is authorized with jwt or
          // the route is under webhooks
  ...
  return plugins.map(async pluginName => {
    await server.register({
      ...
      options : {...(crumbOptions[pluginName] || {})}
    });
  });
}
 ...
 * @param  {String}    config.scopedPackage           Package name with scope
 *     (optional).
...
function registerNotificationEvent(config, server) {
  ...
  Object.keys(notificationConfig).forEach(plugin => {
    if (plugin !== 'options') {
      const Plugin =
          requireNotificationPlugin(notificationConfig[plugin], plugin);
      ...
      notificationPlugin.events.forEach(event => {
        server.events.on(
            event, buildData => notificationPlugin.notify(event, buildData));
      });
    }
  });
}
